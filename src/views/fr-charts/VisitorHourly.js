   ...
   async mounted() {
      if(Array.isArray(this.dt)) {
         ...
         Object.values(this.dt).forEach((elm) => {
            // console.log('Time: ' + new Date(String(elm.hour).split(', ')[1] + " " + String(elm.hour).split(', ')[0]).toLocaleTimeString());
            // days.push(new Date(String(elm.hour).split(', ')[1] + " " + String(elm.hour).split(', ')[0]).toLocaleTimeString());
            days.push(elm.hour);
