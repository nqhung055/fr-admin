<template>
  <div>
    ...
    >
      <v-card>
        <v-card-title>
          <span class="headline">{{ $t("message.editCompany") }}</span>
        </v-card-title>
        <v-card-text>
            ...
            <v-row>
                <v-col cols="10">
                  <v-textarea
                    ...
                    hide-details
                    v-model.trim="editCompany.description"
                    height="auto"
                  />
                </v-col>
              </v-row>
        ...
        <v-card-actions class="pa-4">
          <v-spacer></v-spacer>
          <v-btn class="px-4" color="success" v-on:click="editedCompany">{{
            $t("message.editBtn")
          }}</v-btn>
  ...
  methods: {
    async editedCompany() {        
      const editResponse = await this.$axios.patch(
        `${AppConfig.ip}${AppConfig.api_port}/companies/${this.editCompany.id}?sn=${this.editCompany.shortName}`,
        this.editCompany
      );
